# Target Profile: Pocket Monsters FireRed

## Known repository scope

- Repository: `SakuraiTsubaki/PocketMonsters-FireRed-Decompilation`
- Working target name: Pocket Monsters FireRed
- Platform family: Game Boy Advance
- Series generation: Generation III
- Exact release, region, revision, and build: **Japanese retail cartridge, `BPRJ`, revision 0, identified**

The selected reconstruction target is the locally observed Japanese retail cartridge (`BPRJ`, revision 0). Its hashes and header identity are recorded in `config/target.json`; the status remains `identified`, not independently `verified`.
